...
public class UserAcceptedPickupsActivity  extends AppCompatActivity {
    ListView list_view;
    ...
    public void serverData(){
        ...
        progressDialog = new ProgressDialog(UserAcceptedPickupsActivity.this);
        progressDialog.setMessage("Loading....");
        progressDialog.show();
        ApiService service = RetroClient.getRetrofitInstance().create(ApiService.class);
        Call<List<UserPickupsPojo>> call = service.getUserPickupDetails(sharedPreferences.getString("user_uname","-"),getIntent().getStringExtra("status"));
        call.enqueue(new Callback<List<UserPickupsPojo>>() {
            ...
            public void onResponse(Call<List<UserPickupsPojo>> call, Response<List<UserPickupsPojo>> response) {
                ...
                if(response.body()==null){
                    Toast.makeText(UserAcceptedPickupsActivity.this,"No data found",Toast.LENGTH_SHORT).show();
                }else {
                    userPickups = response.body();
                    list_view.setAdapter(new UserAcceptedPickupsAdapters(userPickups, UserAcceptedPickupsActivity.this));
                }
            }
            ...
            public void onFailure(Call<List<UserPickupsPojo>> call, Throwable t) {
                progressDialog.dismiss();
                Toast.makeText(UserAcceptedPickupsActivity.this, "Something went wrong...Please try later!", Toast.LENGTH_SHORT).show();
            }
        });
    }
    @Override
